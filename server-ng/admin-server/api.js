...
const get_hashed_password = require('../shared/utils/crypto-utils.js').get_hashed_password;
const get_secure_random_string = require('../shared/utils/crypto-utils.js').get_secure_random_string;
...
var sessions_settings_object = {
    ...
    cookie: {
        httpOnly: true,
        secure: !isDevelopment // Only require secure cookies in production
    }
}
...
async function set_up_api_server(app) {
    ...
    app.use('/admin/assets', express.static(
    	path.join(__dirname, '../../front-end-vue3/dist/assets'),
    	{
    		setHeaders: function (res, path, stat) {
    			res.set("Content-Security-Policy", "default-src 'none'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self' data:; connect-src 'self'; manifest-src 'self'");
    		},
    	},
    ));
    ...
    */
    ...
    app.put(constants.API_BASE_PATH + 'settings', validate({ body: UpdateConfigSchema }), async (req, res) => {
        ...
        if(req.body.send_discord_alerts !== undefined) {
            await update_settings_value(
                ...
                req.body.send_discord_alerts.toString()
            );
        }
    ...
    /*
        PAYLOAD CONSOLE API ENDPOINTS
        All endpoints require authentication (except active payloads for probe)
    */

    // Get all payloads (authenticated)
    app.get(constants.API_BASE_PATH + 'payloads', async (req, res) => {
    ...
    // Get single payload by ID (authenticated)
    app.get(constants.API_BASE_PATH + 'payloads/:id', async (req, res) => {
        if (!req.session.authenticated) {
            res.status(401).json({
                "success": false,
                "error": "You must be authenticated to access this resource."
    ...
    // Create new payload (authenticated)
    ...
    app.post(constants.API_BASE_PATH + 'payloads', validate({ body: CreatePayloadSchema }), async (req, res) => {
        if (!req.session.authenticated) {
            res.status(401).json({
                "success": false,
                "error": "You must be authenticated to access this resource."
        ...
        try {
            ...
            const minifiedCode = await minifyAndEncodePayload(req.body.code);
    ...
    app.put(constants.API_BASE_PATH + 'payloads/:id', validate({ body: UpdatePayloadSchema }), async (req, res) => {
        if (!req.session.authenticated) {
            res.status(401).json({
                "success": false,
                "error": "You must be authenticated to access this resource."
        ...
        try {
            ...
            // If code is being updated, minify and encode it
            if (req.body.code) {
    ...
    // Delete payload (authenticated)
    app.delete(constants.API_BASE_PATH + 'payloads/:id', async (req, res) => {
        if (!req.session.authenticated) {
            res.status(401).json({
                "success": false,
                "error": "You must be authenticated to access this resource."
    ...
    // Generate test token for payload testing (authenticated)
    app.post(constants.API_BASE_PATH + 'generate-test-token', async (req, res) => {
        if (!req.session.authenticated) {
            res.status(401).json({
                "success": false,
                "error": "You must be authenticated to access this resource."
        ...
        // Store token with timestamp in app.js's testTokens Map
        // We need to pass this through the app object
