...
const get_secure_random_string = require('./utils/crypto-utils.js').get_secure_random_string;
const get_hashed_password = require('./utils/crypto-utils.js').get_hashed_password;
...
PayloadFireResults.init({
    ...
    // Custom data from payload scripts
    custom_data: {
        type: Sequelize.JSON,
        allowNull: true,
        unique: false
...
/*
	Authentication tokens for probe callbacks
*/
class ProbeTokens extends Model {}
ProbeTokens.init({
	id: {
		allowNull: false,
		primaryKey: true,
	...
	// One-time probe secret for initial js_callback
	probe_secret: {
		type: Sequelize.STRING(64),
		allowNull: false,
		unique: true
	},
	// Has this probe secret been used?
	used: {
		type: Sequelize.BOOLEAN,
		defaultValue: false,
		allowNull: false
	},
	// Session token for background callbacks (generated after first use)
	session_token: {
		type: Sequelize.STRING(64),
		allowNull: true,
		unique: true
	},
	// Associated payload fire ID (set after first use)
	payload_fire_id: {
		type: Sequelize.UUID,
		allowNull: true
	},
	// IP address that requested the probe
	request_ip: {
		type: Sequelize.TEXT,
		allowNull: true
	},
